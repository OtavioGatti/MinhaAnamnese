...
test('getDiscountedPlanAmount calcula o preço com desconto arredondado', () => {
  ...
  assert.equal(getDiscountedPlanAmount(monthly, 0.1), 17.01);
  assert.equal(getDiscountedPlanAmount(semiannual, 0.1), 89.91);
  ...
  assert.equal(getDiscountedPlanAmount(semiannual, 0.9), 49.95);
});

test('getExpectedChargeAmounts reúne preço cheio, descontos e assinatura sem duplicar', () => {
  const semiannual = getBillingPlan('semiannual');

  assert.deepEqual(getExpectedChargeAmounts(semiannual), [99.9]);
  assert.deepEqual(
...
test('isExpectedChargedAmount aceita preço cheio e com desconto de afiliado', () => {
  ...
  assert.equal(isExpectedChargedAmount(monthly, 18.9), true);
  assert.equal(isExpectedChargedAmount(monthly, 17.01), false);

  // Com taxa do afiliado (registro no banco).
  assert.equal(isExpectedChargedAmount(monthly, 17.01, { affiliateDiscountRate: 0.1 }), true);
  assert.equal(isExpectedChargedAmount(semiannual, 89.91, { affiliateDiscountRate: 0.1 }), true);

  // Com taxa da metadata criada pelo nosso checkout.
  assert.equal(isExpectedChargedAmount(semiannual, 89.91, { metadataDiscountRate: 0.1 }), true);

  // Valor da assinatura persistida server-side (protege renovações se a taxa mudar).
  assert.equal(isExpectedChargedAmount(monthly, 17.01, { subscriptionAmount: 17.01 }), true);
