...
function getSyncSecretFromRequest(req) {
  return getHeaderValue(req, 'x-snippet-sync-secret').trim() || getBearerToken(req);
}

function getExpectedSyncSecret() {
  return process.env.SNIPPET_SYNC_SECRET || process.env.ADMIN_SYNC_SECRET || '';
}
...
module.exports = async function handler(req, res) {
  ...
  if (!getExpectedSyncSecret()) {
    return res.status(503).json({
      success: false,
      error: 'Sincronização de frases prontas não configurada.',
    });
  }
  ...
  let result;
  try {
    result = await syncNotionSnippets();
  } catch (error) {
    ...
    if (isNotionAccessError) {
      return res.status(502).json({
        success: false,
        error: 'A integração do Notion usada pelo backend não tem acesso à tabela de Frases Prontas.',
    ...
    return res.status(statusCode >= 400 && statusCode < 600 ? statusCode : 500).json({
      success: false,
      error: 'Falha ao sincronizar Frases Prontas.',
