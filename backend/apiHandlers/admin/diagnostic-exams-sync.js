// Sync manual/agendado do CMS de exames complementares (Notion -> Supabase).
...
} = require('../../services/notionDiagnosticExamsSync');
const {
...
module.exports = async function handler(req, res) {
  ...
  if (!isNotionExamsSyncConfigured()) {
  ...
  try {
    ...
    const data = await syncNotionDiagnosticExams({ bypassReviewGate: true });
    ...
    if (isNotionAccessError) {
      return res.status(502).json({
        success: false,
        error: 'A integração do Notion usada pelo backend não tem acesso à tabela de Exames.',
    ...
    return res.status(error.statusCode || 500).json({
      success: false,
      error: 'Falha ao sincronizar os Exames Complementares.',
