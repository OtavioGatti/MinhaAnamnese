// Rota de POLLING da automação de bulário, chamada por um agendador (ex.: Render
// Cron). Processa páginas com Status Automação em {a gerar, a corrigir}. Aceita
// GET e POST; suporta dryRun (não escreve) e limit.

const { runClinicalDrugAutomation } = require('../../services/clinicalDrugAutomationRunner');
const {
...
module.exports = async function handler(req, res) {
  ...
  const rateLimit = await consumeRateLimit({
    req,
    scope: 'clinical_drug_automation_run',
  ...
  try {
    const result = await runClinicalDrugAutomation({ limit, dryRun });
    return res.status(200).json({ success: true, data: result });
  } catch (error) {
    const statusCode = Number(error?.statusCode) || 500;
    const safeMessage = statusCode < 500 ? error.message : 'Falha ao rodar a automação de bulário.';
