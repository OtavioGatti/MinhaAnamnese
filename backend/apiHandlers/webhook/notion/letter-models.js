...
function isRelevantLetterModelEvent(body) {
  if (!HANDLED_EVENT_TYPES.has(body?.type)) {
    return false;
  }

  const configuredDataSourceId = normalizeNotionId(getNotionLetterModelsConfig().dataSourceId);
  const entityId = normalizeNotionId(body?.entity?.id);
  const parentDataSourceId = normalizeNotionId(
    body?.data?.parent?.data_source_id || body?.data?.data_source_id,
  );
  ...
  return entityId === configuredDataSourceId || parentDataSourceId === configuredDataSourceId;
}
...
module.exports = async function handler(req, res) {
  ...
  if (!isRelevantLetterModelEvent(req.body)) {
    return res.status(200).json({ success: true, data: { ignored: true } });
  }

  const result = await syncNotionLetterModels();
