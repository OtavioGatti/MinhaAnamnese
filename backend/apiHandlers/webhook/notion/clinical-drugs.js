...
function getNotionWebhookVerificationToken() {
  return process.env.NOTION_CLINICO_REVISADO_WEBHOOK_VERIFICATION_TOKEN ||
...
function isRelevantClinicalDrugEvent(body) {
  if (!HANDLED_EVENT_TYPES.has(body?.type)) {
    return false;
  }

  const configuredDataSourceId = normalizeNotionId(getNotionClinicalDrugsConfig().dataSourceId);
...
module.exports = async function handler(req, res) {
  ...
  if (!getNotionWebhookVerificationToken()) {
    return res.status(503).json({ success: false, error: 'Webhook do Notion não configurado.' });
  }
  ...
  if (!isNotionClinicalDrugsSyncConfigured()) {
    return res.status(503).json({ success: false, error: 'Integração Notion -> Supabase do bulário não configurada.' });
  }

  if (!isRelevantClinicalDrugEvent(req.body)) {
